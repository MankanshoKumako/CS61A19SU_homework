...
def balanced(m):
    ...
    if is_weight(left_side) and is_weight(right_side):
        return (size(left_side_end)*left_side_length==size(right_side_end)*right_side_length)
    left_total_weight=total_weight(left_side)
    right_total_weight=total_weight(right_side)
    if left_total_weight*left_side_length!=right_total_weight*right_side_length:
        return False
    balanced_lst=[]
    if is_mobile(left_side):
        balanced_lst+=balanced(left_side)
    if is_mobile(right_side):
        balanced_lst+=balanced(right_side)
    if all(balanced_lst):
        return True
    else:
        return False
