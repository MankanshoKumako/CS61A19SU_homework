...
def make_withdraw(balance, password):
    ...
    count=0
    attempts=[]
    def withdraw(account,user_input):
        nonlocal count,balance,attempts
        if count>2:
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock==0:
            print(f'Machine is out of stock. Here is your ${self.balance}')
        elif self.balance<self.price:
            print(f'You must deposit ${self.price-self.balance} more.')
        elif self.balance==self.price:
            self.stock -= 1
            self.balance=0
